...
impl Bsp {
    ...
    pub fn new(device: &wgpu::Device, queue: &wgpu::Queue, layout: &wgpu::BindGroupLayout, light_layout: &wgpu::BindGroupLayout) -> Bsp {
        ...
        let light_material = Material { diffuse_texture, bind_group, tex_type: -1, transparent: false };
        ...
        for i in 0..nodes.len() {
            ...
            clip_nodes_loaded.push(ClipNode2 { plane_num: nodes[i].plane_num, children: nodes[i].children });
        }
        ...
        get_hulls(&mut hulls, 0, 0, clip_nodes[0].len());

        for i in 0..models.len() {
            hulls.push(Hull {clip_nodes_id: 0, planes_id: 0, first_clip_node: models[i].node_id0, last_clip_node: clip_nodes[0].len() as i32 - 1, mins: models[i].bound.min, maxs: models[i].bound.max});
            hulls.push(Hull {clip_nodes_id: 0, planes_id: 0, first_clip_node: models[i].node_id1, last_clip_node: clip_nodes[0].len() as i32 - 1, mins: models[i].bound.min, maxs: models[i].bound.max});
            hulls.push(Hull {clip_nodes_id: 0, planes_id: 0, first_clip_node: models[i].node_id2, last_clip_node: clip_nodes[0].len() as i32 - 1, mins: models[i].bound.min, maxs: models[i].bound.max});
            hulls.push(Hull {clip_nodes_id: 0, planes_id: 0, first_clip_node: models[i].node_id3, last_clip_node: clip_nodes[0].len() as i32 - 1, mins: models[i].bound.min, maxs: models[i].bound.max})
        }
    ...
    pub fn player_trace(&mut self, start_pos: cgmath::Vector3<f32>, end_pos: cgmath::Vector3<f32>) -> Trace {
        ...
        for i in 0..1 {
            ...
            let mut offset = to_vec3([0.0, 0.0, 0.0]);
            if i > 2 {
                offset = to_vec3(self.models[i - 3].origin);
            }
            ...
            if i < 3 {
                self.recusive_hull_check(self.hulls[i], self.hulls[i].first_clip_node, 0.0, 1.0, start_l, end_l, &mut trace);
            }
            else {
                //self.recusive_hull_check(self.hulls[(i - 2) * 4], self.hulls[(i - 2) * 4].first_clip_node, 0.0, 1.0, start_l, end_l, &mut trace);
            }
    ...
    pub fn recusive_hull_check(&mut self, hull: Hull, num: i32, p1f: f32, p2f: f32, p1: cgmath::Vector3<f32>, p2: cgmath::Vector3<f32>, trace: &mut Trace) -> bool {
        ...
        if num < hull.first_clip_node || num > hull.last_clip_node {
            panic!("RECUSIVE HULL CHECK BAD NODE NUMBER");
        }

        let node = self.clip_nodes[hull.clip_nodes_id as usize][num as usize];
        ...
        if t1 >= 0.0 && t2 >= 0.0 {
            return self.recusive_hull_check(hull, node.children[0], p1f, p2f, p1, p2, trace);
        }
        if t1 < 0.0 && t2 < 0.0 {
            return self.recusive_hull_check(hull, node.children[1], p1f, p2f, p1, p2, trace);
        }
        ...
        if (!self.recusive_hull_check(hull, side_n, p1f, midf, p1, mid, trace)) {
            return false;
        }
        ...
        if self.hull_point_contents(&hull, side_tn, mid) != CONTENTS_SOLID {
            return self.recusive_hull_check(hull, side_tn, midf, p2f, mid, p2, trace);
        }
        ...
        while self.hull_point_contents(&hull, hull.first_clip_node, mid) == CONTENTS_SOLID {
            frac -= 0.1;
            if frac < 0.0 {
                trace.fraction = midf;
                trace.end_pos = mid;
    ...
    pub fn point_contents(&mut self, p: cgmath::Vector3<f32>) -> i32 {
        ...
        while num >= 0 {
            ...
            if num < self.hulls[0].first_clip_node || num > self.hulls[0].last_clip_node {
                panic!("HULL POINT CONTENTS BAD NODE NUMBER");
            }

            let node = self.clip_nodes[0][num as usize];
            let plane = self.planes[node.plane_num as usize];
    ...
    pub fn hull_point_contents(&self, hull: &Hull, num: i32, p: cgmath::Vector3<f32>) -> i32 {

        let mut t_num = num;

        while t_num >= 0 {
            if t_num < hull.first_clip_node || t_num > hull.last_clip_node {
                panic!("HULL POINT CONTENTS BAD NODE NUMBER");
            }

            let node = self.clip_nodes[hull.clip_nodes_id][t_num as usize];
            let plane = self.planes[node.plane_num as usize];
            ...
            if d < 0.0 {
                t_num = node.children[1] as i32;
            }
            else {
                t_num = node.children[0] as i32;
            }
        }
    ...
    pub fn test_player_position(&self, pos: cgmath::Vector3<f32>) -> bool {

        for i in 0..self.hulls.len() {

            let offset = to_vec3(self.models[0].origin);
            let test = pos - offset;

            if self.hull_point_contents(&self.hulls[i], self.hulls[i].first_clip_node, test) == CONTENTS_SOLID {
                return false
            }
        }
...
pub fn load_entities(data: &str) {

    let mut data_s = data.to_owned();

    loop {
        let (data_s_t, com_token) = parse_ents(&data_s);
        data_s = data_s_t;
        if data_s.len() == 0 {
            break;
        }
        if com_token[0] != '{' {
            panic!("Didnt find opening brace");
        }
    }
}

pub fn parse_ents(data: &str) -> (String, Vec<char>) {

    let mut tracker: usize = 0;

    let mut com_token = Vec::new();

    let mut out_str = String::new();

    if data.len() == 0 {
        return (out_str, com_token);
    }

    loop {
        let val = repe(data, &mut tracker, &mut com_token);
        if val == 0 {
            return (out_str, com_token);
        }
        else if val == 2 {
    ...
    for i in 0..tracker {
        out_str.push(data.as_bytes()[i] as char);
    }

    (out_str, com_token)
}

pub fn repe(data: &str, tracker: &mut usize, com_token: &mut Vec<char>) -> i32 {

    while data.as_bytes()[*tracker] <= ' ' as u8 {
        *tracker += 1;
        if *tracker == data.len() {
            return 0;
        }
    ...
    if data.as_bytes()[*tracker] as char == '/' && data.as_bytes()[*tracker + 1] as char == '/' {

        while data.as_bytes()[*tracker] as char != '\n' {
    ...
    if data.as_bytes()[*tracker] as char == '\"' {

        *tracker += 1;
        loop {
            *tracker += 1;
            if data.len() == *tracker {
                com_token.push(0 as char);
                return 2;
            }
            if data.as_bytes()[*tracker] as char == '\"' {
                com_token.push(0 as char);
            ...
            com_token.push(data.as_bytes()[*tracker] as char);
        }
    }

    let mut c = data.as_bytes()[*tracker] as char;
    ...
    loop {
        com_token.push(c);
        *tracker += 1;
        c = data.as_bytes()[*tracker] as char;
        if c == '{' || c == '}' || c == '(' || c == ')' || c == '\'' || c == ':' {
            break;
        }
        if (c as u8) < 33 {
